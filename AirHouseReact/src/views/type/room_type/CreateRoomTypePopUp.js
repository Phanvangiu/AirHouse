...
//api import
import { CreateRoomTypeMutation } from "api/room-typeApi";
...
export default function CreateRoomTypePopUp({ currentPage, setShowPopUp }) {
  const queryClient = useQueryClient();
  const createMutation = CreateRoomTypeMutation();
  ...
  return (
    <StyledPopUpContainer setShowPopUp={turnOffPopUp}>
      <StyledForm>
        ...
        <StyledButtonRow>
          <button disabled={roomTypeName == "" || imgSrc == DefaultImg} onClick={onAddNewRoomType} className="submit-button">
            Submit
          </button>
        </StyledButtonRow>
      </StyledForm>
