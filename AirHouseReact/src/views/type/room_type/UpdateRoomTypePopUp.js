...
//api import
import { UpdateRoomTypeMutation } from "api/room-typeApi";
import { RoomTypeQueryId } from "api/room-typeApi";
...
export default function UpdateRoomTypePopUp({ currentPage, chosenId, setShowPopUp }) {
  const queryClient = useQueryClient();
  const roomTypeQuery = RoomTypeQueryId(chosenId);
  const updateMutation = UpdateRoomTypeMutation();
  ...
  useEffect(() => {
    if (roomTypeQuery.isSuccess) {
      setImgSrc(roomTypeQuery.data[0].icon_image);
      setRoomTypeName(roomTypeQuery.data[0].name);
    }
  }, [roomTypeQuery.status]);
  ...
  const onUpdateEvent = (ev) => {
    ...
    formData.append("id", roomTypeQuery.data[0].id);
  ...
  return (
    <StyledPopUpContainer setShowPopUp={turnOffPopUp}>
      <StyledForm>
        <h6>{roomTypeQuery.isSuccess ? "ID: " + roomTypeQuery.data[0].id : <Skeleton />}</h6>
        <StyledInputField>
          <label>Room Type Name</label>
          <input
            onChange={(ev) => {
              setRoomTypeName(ev.target.value);
            }}
            type="text"
        ...
        <StyledButtonRow>
          <button onClick={onUpdateEvent} disabled={roomTypeName == "" || imgSrc == DefaultImg} className="submit-button">
            Update
          </button>
        </StyledButtonRow>
      </StyledForm>
