...
import { ReportFeeQuery } from "api/reportApi";
...
export default function BookingFeeReport() {
  ...
  const reportFeeQuery = ReportFeeQuery(year);
  const reportYearQuery = ReportFeeQuery(null);
  ...
  useEffect(() => {
    if (reportFeeQuery.isSuccess) {
      if (reportFeeQuery.data.type == "total") {
        setData({
          labels: reportFeeQuery.data.report.map((data) => data.year),
          datasets: [
            {
              label: "Booking Fees Report",
              data: reportFeeQuery.data.report.map((data) => data.total),
      ...
      if (reportFeeQuery.data.type == "year") {
        setData({
          labels: reportFeeQuery.data.report.map((data) => data.month),
          datasets: [
            {
              label: "Booking Fees Report",
              data: reportFeeQuery.data.report.map((data) => data.total),
