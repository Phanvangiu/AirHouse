...
export default function BookingItem(props) {
  ...
  const onClickProperty = () => {
    navigate({
      pathname: "/property",
      search: `?id=${props.BookingItem.property_id}`,
    });
  };

  return (
    <Box>
      <div onClick={onClickProperty} className="item1">
      ...
      <div className="item2">
        ...
        <p>
          ...
          {props.BookingItem.province}, {props.BookingItem.districts},{" "}
          {props.BookingItem.Property_Address}
        </p>
        <p>
          <FontAwesomeIcon className="bar" icon={faAddressBook} />
          {props.BookingItem.check_in_date} <span className="to">to</span>{" "}
          {props.BookingItem.check_out_date}
        </p>
        {props.BookingItem.status == "accepted" && (
          <StyledPay>
            <FontAwesomeIcon icon={faMoneyBill} /> Amount Pay:{" "}
            {Number(props.BookingItem.site_fees) +
              Number(props.BookingItem.price_for_stay)}
            $
          </StyledPay>
        )}
        {props.BookingItem.status == "accepted" && (
          <StyledAlert>
            Booking will be expired after 24 hours from{" "}
            {props.BookingItem.created_at}
          </StyledAlert>
        )}
      </div>
      <div className="item3">
